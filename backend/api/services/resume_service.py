...
class ResumeService:
    ...
    async def analyze_resume(self, resume_text: str, user_context: dict) -> dict:
        ...
        message = (
            f"## Resume Content\n{resume_text[:5000]}\n\n"
            f"## Instructions\n"
            f"Analyze this resume for interview coaching purposes. Return JSON with:\n"
            f"- positioning_strengths: what the resume communicates well\n"
            f"- likely_concerns: what an interviewer might question\n"
            f"- career_narrative_gaps: where the career story has gaps or unclear transitions\n"
            f"- story_seeds: list of potential storybank stories that can be extracted "
            f"(objects with 'title', 'source_bullet', 'potential_skill')\n"
            f"- ats_compatibility: assessment of ATS keyword optimization\n"
            f"- recruiter_scan: what a recruiter sees in a 6-second scan\n"
            f"- bullet_quality: assessment of bullet point specificity and impact\n"
            f"- seniority_calibration: whether the resume reads at the right level for target roles\n"
            f"- keyword_coverage: how well it covers target role keywords\n"
            f"- top_fixes: ordered list of the 5 highest-impact changes (objects with 'fix', 'impact', 'effort')"
        )
